...
<body>
	...
	<main role="main" class="container">
		<div class="d-flex align-items-center p-3 my-3 text-white-50 bg-purple rounded box-shadow" style ="width:200%;">
			...
			<div class="lh-100">
				<h6 class="mb-0 text-white lh-100">연금저축상품 조회 결과</h6>
    ...
    <script type="text/javascript" charset="euc-kr">
        $(document).ready(function() {
            $.ajax({
                ...
                type: "GET",
                dataType: "xml",
                url: "http://finlife.fss.or.kr/finlifeapi/annuitySavingProductsSearch.xml?auth=7ca1f03b6f18f09837b676a3f0f2370c&topFinGrpNo=060000&pageNo=1",
                ...
                success: function(res) {
                    ...
                    $(xml).each(function() {
                        ...
                        var pnsn_recp_trm_nm = $(this).find('options').find('option').find('pnsn_recp_trm_nm').text().substring(0, 6);
                        var pnsn_entr_age_nm = $(this).find('options').find('option').find('pnsn_entr_age_nm').text().substring(0, 3);       
                        var mon_paym_atm_nm = $(this).find('options').find('option').find('mon_paym_atm_nm').text().substring(0, 8);
                        var paym_prd_nm = $(this).find('options').find('option').find('paym_prd_nm').text().substring(0, 3);
                        var pnsn_strt_age_nm = $(this).find('options').find('option').find('pnsn_strt_age_nm').text().substring(0, 3);
                        var pnsn_recp_amt = $(this).find('options').find('option').find('pnsn_recp_amt').text().substring(0, 6);
                        ...
                        str += '<th>' + join_way + '</th>\n';
                        str += '<th>' + pnsn_recp_trm_nm + '</th>\n';
                        str += '<th>' + pnsn_entr_age_nm + '</th>\n';
                        str += '<th>' + mon_paym_atm_nm + '</th>\n';
                        str += '<th>' + paym_prd_nm + '</th>\n';
                        str += '<th>' + pnsn_strt_age_nm + '</th>\n';
                        str += '<th>' + pnsn_recp_amt + '</th>\n';
                        str += '</tr>\n';
                    });
                    str += '</tbody>'
                    $('#bank_table').html(str);
